...
def aggregateNew(results: List[Tuple[NDArrays, int, dict]],client_id) -> NDArrays:
    ...
    weighted_weights = [
        [layer * 1 for layer in weights] for weights, num_examples, _ in results
    ]
    ...
    metrics = [mtrcs for _,mtrcs in sorted(zip(client_id,[m for _, _, m in results]),key=lambda pair: pair[0])]
